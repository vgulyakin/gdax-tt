...
class GDAXExchangeAPI {
    constructor(options) {
        ...
        this.publicClient = new gdax_1.PublicClient(this._apiURL);
    }
    get apiURL() {
        return this._apiURL;
    }
    ...
    loadCandles(options) {
        ...
        return this.publicClient.getProductHistoricRates(product, {
            ...
            return Promise.reject(new errors_1.HTTPError(`Error loading ${product} candles from GDAX`, errors_1.extractResponse(err.response)));
        });
    }
    aggregateBook(body) {
        ...
        switch (order.orderType) {
            ...
            case 'stop':
                gdaxOrder = {
                    ...
                    client_oid: order.clientId,
                    stp: order.extra && order.extra.stp
                }; // Override for incomplete definition in GDAX lib
                break;
            default:
                return Promise.reject(new errors_1.GTTError(`Invalid order type: ${order.type}`));
        }
        return this.authClient.placeOrder(gdaxOrder).then((result) => {
            // Check for error
    ...
    loadAllOrders(product) {
        ...
        const loop = (after) => {
            return self.loadNextOrders(product, after).then((result) => {
                const liveOrders = result.orders.map(GDAXOrderInfoToOrder);
...
function GDAXOrderResultToOrder(order) {
    let size;
    let price;
    if (+order.size > 0) {
...
function GDAXOrderInfoToOrder(order) {
    let size;
    let price;
    if (+order.size > 0) {
        size = types_1.Big(order.size);
